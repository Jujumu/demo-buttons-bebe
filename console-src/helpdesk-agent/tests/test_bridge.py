...
class SendReplyTests(unittest.TestCase):
    ...
    def test_confirmation_and_seed_refused(self) -> None:
        ...
        self.assertEqual(unconfirmed["error"], "confirmation_required")
        seed = handle_http(
            TOOL_SEND_REPLY,
            {"ticketId": "t-ada-track", "text": "Hi", "confirmed": True},
            actor="human",
        )
        self.assertEqual(seed["error"], "no_real_recipient")
    ...
    def test_allowlist_blocks(self) -> None:
        ...
        self.assertEqual(blocked["error"], "recipient_not_allowed")

    def test_email_route_when_switch_off(self) -> None:
        ticket_id = self._intake(source="agentmail")
        self.assertEqual(reply_route(dispatch("helpdesk.get_ticket", {"ticketId": ticket_id})["ticket"]), "email")
        with patch("bridge.email_out.send_email_reply") as send:
            send.return_value = {"ok": True, "messageId": "am-1", "deliveryStatus": "sent"}
            payload = handle_http(
                TOOL_SEND_REPLY,
            ...
            self.assertTrue(payload["ok"])
            self.assertEqual(payload["via"], "email")
            send.assert_called_once()
            ticket = payload["ticket"]
            self.assertEqual(ticket["messages"][-1]["via"], "email")

    def test_gorgias_route_lazy_and_sends(self) -> None:
        os.environ["GORGIAS_BRIDGE_ENABLED"] = "1"
        os.environ["GORGIAS_SUBDOMAIN"] = "demo"
        os.environ["GORGIAS_API_EMAIL"] = "agent@example.com"
        os.environ["GORGIAS_API_KEY"] = "key"
        # Drop a prior import so we can prove the handler imports lazily.
        ...
        self.assertNotIn("bridge.gorgias_api", sys.modules)
        with patch("bridge.gorgias_api.send_public_reply") as send:
            send.return_value = {"ok": True, "messageId": "99", "deliveryStatus": "sent"}
            payload = handle_http(
                TOOL_SEND_REPLY,
            ...
            self.assertTrue(payload["ok"])
            self.assertEqual(payload["via"], "gorgias")
            send.assert_called_once()
            self.assertIn("bridge.gorgias_api", sys.modules)

    def test_gorgias_module_not_loaded_when_email_route(self) -> None:
        before = {k for k in sys.modules if "gorgias_api" in k}
        ticket_id = self._intake(source="agentmail")
        with patch("bridge.email_out.send_email_reply", return_value={"ok": True, "messageId": "1", "deliveryStatus": "sent"}):
            handle_http(
                TOOL_SEND_REPLY,
