...
# Force-closed even if a production .env on this host has these on.
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            body = {}
        tool = body.get("tool")
        if tool == "helpdesk.send_reply":
            self._json(200, {
                "ok": False,
                "error": SEND_ACCESS_ERROR,
                "message": ACTIVATE_SEND_MESSAGE,
                "tool": tool,
            })
            return
        payload = handle_http(tool, body.get("arguments") or {}, actor="human")
