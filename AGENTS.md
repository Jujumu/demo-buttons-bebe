...
- **Ticket row (Clerk):** `id`, `customerName`, `subject`, `snippet`, `status`, `updatedAt`, `customerId`, `orderId`, `requestType`. `customerName` is intake From, never `Customer.displayName`. Ticket `status` is helpdesk `open` / `closed` / `snoozed`, not `Return.status`. `requestType` is first-party (`marketing_unsubscribe` or `null`), not a Shopify consent write.
