...
Respect the safety model. This agent is assistive only: prepare a draft for a human to review — never send a reply or post an internal note automatically. Return every complete draft to the processor between `<DRAFT>` and `</DRAFT>` so it can appear in the console Ticket feed. The agent ALWAYS generates a draft, even for sensitive topics. If the top `search_kb` results are marked sensitive (for example refunds, chargebacks, payment disputes, damaged / wrong / missing items, cancellations, address changes, or an angry customer), draft a reply prefixed with `[SENSITIVE — REVIEW CAREFULLY BEFORE SENDING]` and use safe acknowledgment language (no money promises, no refund confirmations). The human agent reviews every draft and may send, post as an internal note, request a rewrite, edit, or discard it — the human is the safety gate. Never promise, confirm, or process a refund in any draft.
...
If you ever change the system, update `/opt/buttonsbebe/CLAUDE.md` so it stays the single source of truth.
