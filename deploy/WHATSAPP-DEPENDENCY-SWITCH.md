...
On a detected failure, the helper stops that same unit, preserves the failed candidate modules under the private backup, restores old modules and byte-identical original package manifests/modes, and starts the old service with only the reviewed startup-log redaction retained. The rollback server SHA and variant are recorded in the private receipt; the unmodified original server stays in the private backup. This prevents a rollback restart from logging the private BASE again, including failures after stop but before candidate files changed. A pre-stop refusal does not edit or restart anything. Before rollback writes, the current server must match either the approved original or approved redacted hash, and both package manifests must match their approved original or candidate hashes; unexpected concurrent code or manifest edits are preserved and the unit stays stopped for manual recovery. Readiness must pass afterward. It never deletes or rewinds auth data—even if pairing data appeared during the operation. SIGINT/SIGTERM enter the same error path. If rollback itself fails, the journal marks `manual-recovery-required`; do not infer success from a restarted process alone.

The private `switch.json` journal records phases, reviewed hashes, and original modes. The successful receipt supplies its recovery directory ID under `/opt/buttonsbebe/backups/`. Rollback deliberately retains that one logging fix; it never changes the BASE value or routing/authentication behavior. Source and module moves are atomic individually, not a whole-operation transaction. A host power loss or SIGKILL requires manual inspection of that journal and preserved files before restarting; do not rerun a stale plan or delete the original module backup. The helper does not rotate secrets or remove historical logs.
