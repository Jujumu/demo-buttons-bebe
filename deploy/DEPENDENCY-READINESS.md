...
Recommended adoption: CI install each lock with `--require-hashes` into separate environments, run actual MCP catalog/schema tests under both, and run KB tests under the KB lock. Change KB setup to consume its hashlock; keep existing live environments untouched until those gates pass and a separately reviewed runtime rebuild is needed. The deploy receiver already classifies requirements.lock as dependency material, so changes should not silently reinstall live dependencies.
