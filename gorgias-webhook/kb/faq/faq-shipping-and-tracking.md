---
title: FAQ — Shipping, Tracking & Delivery
category: faq
status: confirmed
source: derived-from-tickets
tags: [shipping, tracking, delivery, pickup, processing-time, usps, ups, eta]
---

## How long until my order ships? (processing time)

Orders usually ship within **24–48 hours**. This is the **processing time** before
the package leaves the warehouse, and it is separate from carrier delivery time. If
the order is unfulfilled, explain this normal processing time; if it has shipped,
share the tracking.

## How long does shipping take? (carrier time)

Carrier delivery time depends on the method selected at checkout (separate from the
24–48 hour processing time):
- **USPS:** approximately **7–14 days** (can be slow).
- **UPS:** usually **next day**.
- **ETA shipping:** usually **1–2 days**.

If a customer needs it by a specific date, recommend a faster shipping option.

## Where is my order / has it shipped?

This is the single most common question. Look up the order's fulfillment status
and tracking. If it shipped, share the tracking number/link. If it hasn't
shipped yet, let the customer know it's being prepared and the normal 24–48 hour
processing time. Keep it warm and brief.

## Can you ship my order ASAP?

Yes, when the warehouse is open and the item is in stock, agents will prioritize
"please ship ASAP" requests and confirm ("we will ship it asap" / "shipping it
out now"). If the warehouse is already closed for the day, explain kindly that it
couldn't go out today and will ship the next business day. Never promise the
carrier's delivery date.

## Can it arrive by a specific date? / I need it urgently

The warehouse can be asked to **prioritize** the order, but delivery depends on the
shipping method selected at checkout. If a slow method was selected and the order
has not shipped yet, offer an upgraded shipping option. Do not guarantee a delivery
date the carrier controls. See `../intents/intent-18-order-needed-urgently.md`.

## Can I pick up locally / what are the hours?

Yes — local customers can pick up at the store warehouse during posted
hours, and there are bins by the side door available outside business hours
("the bins stay outside by the side door 24 hours a day"). Hours vary and are
sometimes extended, so confirm the day's hours before quoting them.

## Do you ship internationally?

Yes — international customers order regularly (e.g. Canada, Israel). Confirm
current international rates/options before quoting. International customers are
generally responsible for any customs/import fees their country charges.

## I see a charge but never got a confirmation email

Often this is a typo in the email address on the order. Verify the order exists,
correct the email if needed, and share the tracking if it has already shipped.
