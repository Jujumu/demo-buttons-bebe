...
- **`--yolo`** — a Hermes flag that **auto‑approves the AI's tool calls**. The processor **no longer uses it** (DEV-ISSUES #8): it passes `-t buttonsbebe_kb,buttonsbebe_redo,buttonsbebe_gorgias` instead, so only the three read‑only MCP servers are in scope. `HERMES_SKIP_APPROVAL=1` restores it as a temporary unblock and logs a warning every run.
- **toolset** — Hermes' unit of tool permission. Each MCP server registers one named by its configured server key. Passing `-t` names exactly which ones a run may use.
